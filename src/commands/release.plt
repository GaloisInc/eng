...
test(no_exclude_config, [nondet, cleanup(cleanup_eng)]) :-
    cleanup_eng,
...
test(toplevel_not_excluded, [nondet, cleanup(cleanup_eng)]) :-
    cleanup_eng,
...
test(exclude_by_path, [nondet, cleanup(cleanup_eng)]) :-
    cleanup_eng,
    assertz(eng:key(release, exclude)),
...
test(exclude_by_name, [nondet, cleanup(cleanup_eng)]) :-
    cleanup_eng,
...
test(not_in_exclude_list, [nondet, cleanup(cleanup_eng)]) :-
    cleanup_eng,
    assertz(eng:key(release, exclude)),
...
test(multiple_excludes_match, [nondet, cleanup(cleanup_eng)]) :-
    cleanup_eng,
    assertz(eng:key(release, exclude)),
...
test(multiple_excludes_no_match, [nondet, cleanup(cleanup_eng)]) :-
    cleanup_eng,
    assertz(eng:key(release, exclude)),
...
test(exclude_by_name_with_into, [nondet, cleanup(cleanup_eng)]) :-
    cleanup_eng,
...
test(release_cmd_prep_excluded, [nondet, cleanup(cleanup_eng)]) :-
    cleanup_eng,
    assertz(eng:key(release, exclude)),
    assertz(eng:eng(release, exclude, "subproj/foo")),
    Context = context('_eng_', '/project/subproj/foo', 'subproj/foo'),
...
test(release_cmd_post_excluded, [nondet, cleanup(cleanup_eng)]) :-
    cleanup_eng,
    assertz(eng:key(release, exclude)),
    assertz(eng:eng(release, exclude, "subproj/foo")),
    Context = context('_eng_', '/project/subproj/foo', 'subproj/foo'),
...
test(release_cmd_prep_no_excludes_no_prep, [nondet, cleanup(cleanup_eng)]) :-
    cleanup_eng,
...
%% returns success (0), preserving prior behaviour.
test(release_cmd_post_no_excludes_no_post, [nondet, cleanup(cleanup_eng)]) :-
    cleanup_eng,
