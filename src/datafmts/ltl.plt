...
test(negated_expr, [nondet]) :-
    check_parsing("! (shutdown_requested)",
                  op(not(term(ident("shutdown_requested"), boolean)), boolean),
                  "(not shutdown_requested)").

test(z_expr, [nondet]) :-
    check_parsing("(Z (! startup))",
                  op(ltlZ(op(not(term(ident("startup"), boolean)), boolean)), boolean),
                  "ZtoPre((not startup))").

test(h_expr, [nondet]) :-
    check_parsing("(H (! startup))",
                  op(ltlH(op(not(term(ident("startup"), boolean)), boolean)), boolean),
                  "H((not startup))").

test(boolean_or_expr, [nondet]) :-
    check_parsing("((shutdown_running) | startup)",
                  op(or(term(ident("shutdown_running"), boolean),
                        term(ident("startup"), boolean)), boolean),
                  "(shutdown_running or startup)").

test(boolean_triple_term, [nondet]) :-
    check_parsing("stopping & !shutdown_running & !startup",
                  op(and(term(ident("stopping"), boolean),
                         op(not(op(and(term(ident("shutdown_running"), boolean),
                                       op(not(term(ident("startup"), boolean)), boolean)),
                                   boolean)),
                            boolean)),
                     boolean),
                  % surprise!
...
test(boolean_triple_term_parens, [nondet]) :-
    check_parsing("stopping & (!shutdown_running) & (!startup)",
                  op(and(term(ident("stopping"), boolean),
                         op(and(op(not(term(ident("shutdown_running"), boolean)),
                                   boolean),
                                op(not(term(ident("startup"), boolean)), boolean)),
                            boolean)),
                     boolean),
...
test(boolean3_expr, [nondet]) :-
    check_parsing("((shutdown_running) | (startup & (Z (! startup))))",
                  op(or(term(ident("shutdown_running"), boolean),
                        op(and(term(ident("startup"), boolean),
                               op(ltlZ(op(not(term(ident("startup"), boolean)),
                                          boolean)),
                                  boolean)),
                           boolean)),
                     boolean),
...
test(boolean3_expr_extra_parens, [nondet]) :-
    check_parsing("(((((shutdown_running)) | (startup & (((Z (! ((((startup))))))))))))",
                  op(or(term(ident("shutdown_running"), boolean),
                        op(and(term(ident("startup"), boolean),
                               op(ltlZ(op(not(term(ident("startup"), boolean)),
...
test(boolean_opt_9, [nondet]) :-
    check_parsing("(((alarm_enabled & (! FALSE) | (! TRUE) & TRUE | FALSE) & (! (! disable_alarm))) & (! (! (Z (! (! FALSE))))))",
                  ...
                  op(and(op(and(term(ident("alarm_enabled"), boolean),
                                term(ident("disable_alarm"), boolean)), boolean),
...
test(upon_next_expr, [nondet]) :-
    ...
    check_parsing("(H ((Y ((not_y & l) & (Z (! (not_y & l))))) -> ((m & (i >= 0)) | (! (Y TRUE)))))",
                  ...
                  op(ltlH(op(implies(op(ltlY(op(and(op(and(term(ident("not_y"), boolean),
                                                           term(ident("l"), boolean)),
                                                       boolean),
                                                    op(ltlZ(op(not(op(and(term(ident("not_y"), boolean),
                                                                          term(ident("l"), boolean)),
                                                                      boolean)),
                                                               boolean)),
                                                       boolean)),
                                                boolean)),
                                        boolean),
                                     op(or(op(and(term(ident("m"), boolean),
                                                  op(geq(term(ident("i"), integer),
                                                         term(num(0), integer)),
                                                     boolean)),
                                              boolean),
                                           op(ltlZ(term(lit(false), boolean)),
                                              boolean)),
                                        boolean)),
                             boolean)),
                     boolean),
