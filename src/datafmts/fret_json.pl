...
% Remove references to special variables:
%
%   * FFin_$scope_mode$
%   * Fin_$scope_mode$
%   * Lin_$scope_mode$
...
remove_special_vars(TxtIn, TxtOut) :-
    special_var(Var, Repl),
    string_contains(TxtIn, Var), !,
    subst(Var, Repl, TxtIn, N),
    remove_special_vars(N, TxtOut).
remove_special_vars(TxtIn, TxtIn).
special_var("FFin_$scope_mode$", "bad_FFin_TBD_scope_mode_something").
special_var("Fin_$scope_mode$", "bad_Fin_TBD_scope_mode_something").
special_var("Lin_$scope_mode$", "bad_Lin_TBD_scope_mode_something").

fetched_ft_pt_update(Defs, Inp, Out) :-
    % n.b. do not change the order of substitutions here
    get_dict(ft, Defs, FT),
    remove_special_vars(FT, FT_),
    parse_ltl(FT_, FTLTL),
