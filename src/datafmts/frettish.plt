...
test(state_change, [nondet]) :-
    ...
    assertion(PostCond == op(implies( op(and(term(ident("wet"), bool),
...
test(stage_change_natural, [nondet]) :-
    ...
    assertion(PreCond == op(and(term(ident("wet"), bool),
                                term(ident("awake"), bool)),
                            bool)),
    assertion(RegCond == op(and(term(ident("wet"), bool),
                                term(ident("awake"), bool)),
                            bool)),
    fretment_vars(condition, FretMent, CVars),
    ...
    get_dict(post_condition, Responses, PostCond),
    assertion(Rspns == "satisfaction"),
    assertion(PostCond == op(eq(term(ident("noise"), type_unassigned('⚲T2')),
                                term(ident("croaking"), type_unassigned('⚲T2'))),
                             bool)),
    fretment_vars(response, FretMent, RespVars),
...
check_scs_scope(FretMent, ScopeType, ScopeVars, ExpOut, Ranges) :-
    ...
    ; ScopeVars = [SV1⦂SVTy, SV2⦂SVTy],
      assertion(SMode == op(eq(term(ident(SV1), SVTy),
                               term(ident(SV2), SVTy)),
                            bool))
    ),
    fretment_vars(scope, FretMent, SVars),
    ...
    assertion(Condition == "regular"),
    assertion(Qualifier == "upon"),
    assertion(PreCond == term(ident("wet"), bool)),
    assertion(RegCond == term(ident("wet"), bool)),
    fretment_vars(condition, FretMent, CVars),
...
check_scsi_with_timing_common(FretMent, Timing, TimingVars, ExpOut, Ranges) :-
    ...
    get_dict(post_condition, Responses, PostCond),
    assertion(Rspns == "satisfaction"),
    assertion(PostCond == op(eq(term(ident("noise"), type_unassigned('⚲T1')),
...
test(bool_exprs, [nondet]) :-
    ...
    get_dict(scope_mode, SFret, SMode),
    assertion(SMode == op(or(op(or(term(ident("awake"), bool),
    ...
    assertion(Condition == "regular"),
    assertion(Qualifier == "upon"),
    assertion(PreCond == op(or(term(ident("wet"), bool),
                               op(persisted(term(num(3), number),
    ...
    get_dict(post_condition, Responses, PostCond),
    assertion(Rspns == "satisfaction"),
    assertion(PostCond == op(and(op(eq(term(ident("noise"), number),
                                       term(ident("croaking"), number)), bool),
                                 op(or(op(not(term(ident("asleep"), bool)),
                                          bool),
                                       op(and(op(neq(term(ident("noise"), number),
                                                     term(ident("silent"), number)),
                                                 bool),
                                              op(and(op(gt(term(ident("noise"), number),
                                                           term(ident("silent"), number)),
                                                       bool),
                                                     op(and(op(eq(term(ident("noise"), number),
                                                                  op(sub(term(num(307), number),
                                                                         op(add(term(num(5), number),
                                                                                op(divide(term(num(6), number),
                                                                                          op(mul(term(num(1), number),
                                                                                                 op(expo(term(num(2), number),
                                                                                                         term(num(32345), number)),
                                                                                                    number)),
                                                                                             number)),
                                                                                   number)),
                                                                            number)),
...
test(stage_change_scope_invert_until, [nondet]) :-
    ...
    check_scsi_with_timing(FretMent, "until",
                           op(not(term(ident("wet"), bool)), bool),
                           ["wet"⦂bool],
                           ExpOut,
                           ranges{ scopeTextRange:[0, 14],
                                   conditionTextRange:[16, 27],
...
test(with_ltl_predicate, [nondet]) :-
    ...
    assertion(Condition == "holding"),
    assertion(Qualifier == "whenever"),
    assertion(PreCond == op(and(term(ident("awake"), bool),
                                op(persisted(term(num(2), number),
